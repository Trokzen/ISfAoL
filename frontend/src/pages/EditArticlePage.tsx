...
interface Author {
  ...
  user_employee_id?: number | null;  // Добавляем поле для ID сотрудника
}
...
export default function EditArticlePage() {
    ...
    const fetchArticle = async () => {
      try {
        ...
        setAuthors(article.authors.map(author => ({
          ...
          user_employee_id: author.user_employee_id  // Сохраняем ID сотрудника
        })));
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Проверяем, что сумма вкладов не превышает 100%
    const totalContribution = authors.reduce((sum, author) => sum + author.contribution, 0);
    if (totalContribution > 100) {
      setError(`Сумма вкладов авторов не должна превышать 100%. Текущая сумма: ${totalContribution}%. Пожалуйста, уменьшите вклад одного или нескольких авторов.`);
      setLoading(false);
      return;
    }
    ...
    try {
      const articleData: any = {
        ...
        year_pub: yearPub,
        in_rinc: inRinc,
        authors: authors.map(({ id, name, contribution, applied_for_award, award_applied_date, user_employee_id }) => ({
          author_name: name,
          contribution,
          applied_for_award,
          award_applied_date: award_applied_date,
          user_employee_id: user_employee_id || null  // Отправляем ID сотрудника
        }))
      };
